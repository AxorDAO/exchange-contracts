...
import { addressToBytes32, addressesAreEqual, bnToBytes32, combineHexStrings, hashString } from "../lib/BytesHelper";
...
export class Orders {
  ...
  constructor(networkId: number, web3: Web3, address: string, eip712DomainName: string = DEFAULT_EIP712_DOMAIN_NAME) {
  ...
  public getFeeForOrder(amount: BigNumber, isTaker: boolean = true): Fee {
    if (!isTaker) {
  ...
  public async getSignedOrder(order: Order, signingMethod: SigningMethod): Promise<SignedOrder> {
   ...
   */
  public async signOrder(order: Order, signingMethod: SigningMethod): Promise<string> {
    switch (signingMethod) {
      ...
      case SigningMethod.Compatibility:
        ...
        console.log("🚀 ~ file: Orders.ts:207 ~ Orders ~ orderHash:", orderHash);
  ...
  public async signOrderEthers(order: Order, wallet: ethers.Wallet, signingMethod: SigningMethod): Promise<string> {
    const orderHash = this.getOrderHash(order);
    console.log("🚀 ~ file: Orders.ts:207 ~ Orders ~ orderHash:", orderHash);
    // const privateKey = "91c8c12dbf075f1141665dbeea2df91c62ae0296d785552872e4337a2ffd2a81";
    // const wallet = new ethers.Wallet(privateKey);
    console.log("🚀 ~ file: Orders.ts:248 ~ Orders ~ wallet:", wallet.address);
    // const rawSignature = await this.web3.eth.sign(orderHash, order.maker);
    const rawSignature = await wallet.signMessage(Buffer.from(orderHash.slice(2), "hex"));
    const hashSig = createTypedSignature(rawSignature, SIGNATURE_TYPES.DECIMAL);
    if (signingMethod === SigningMethod.Hash) {
      return hashSig;
    }
   ...
   */
  public async signCancelOrder(order: Order, signingMethod: SigningMethod): Promise<string> {
    return this.signCancelOrderByHash(this.getOrderHash(order), order.maker, signingMethod);
  }
   ...
   */
  public async signCancelOrderByHash(orderHash: string, signer: string, signingMethod: SigningMethod): Promise<string> {
    switch (signingMethod) {
      case SigningMethod.Hash:
      case SigningMethod.UnsafeHash:
      case SigningMethod.Compatibility:
  ...
  public orderHasValidSignature(order: SignedOrder): boolean {
    return hashHasValidSignature(this.getOrderHash(order), order.typedSignature, order.maker);
  }

  /**
   * Returns true if the order hash has a non-null valid signature from a particular signer.
   */
  public orderByHashHasValidSignature(orderHash: string, typedSignature: string, expectedSigner: address): boolean {
   ...
   */
  public cancelOrderHasValidSignature(order: Order, typedSignature: string): boolean {
  ...
  public fillToTradeData(order: SignedOrder, amount: BigNumber, price: Price, fee: Fee): string {
    ...
    const fillData = this.web3.eth.abi.encodeParameters(
      ...
      [amount.toFixed(0), price.toSolidity(), fee.toSolidity(), fee.isNegative()],
    );
    return combineHexStrings(orderData, fillData, signatureData);
  }

  public tradeDataLiquidationEncode(amount: BigNumber, isBuy: boolean, allOrNothing: boolean): string {

    return this.web3.eth.abi.encodeParameters(
      ["uint256", "bool", "bool"],
      [amount.toFixed(0), isBuy, allOrNothing],
    );
  }
  ...
  private async ethSignTypedOrderInternal(order: Order, signingMethod: SigningMethod): Promise<TypedSignature> {
    ...
    return ethSignTypedDataInternal(this.web3.currentProvider, order.maker, data, signingMethod);
  }
  ...
  private async ethSignTypedCancelOrderInternal(
    ...
    return ethSignTypedDataInternal(this.web3.currentProvider, signer, data, signingMethod);
  }
